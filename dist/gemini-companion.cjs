...
var require_logging_utils = __commonJS({
  "node_modules/google-logging-utils/build/src/logging-utils.js"(exports2) {
    ...
    var process3 = __importStar(require("node:process"));
    ...
    var DebugLogBackendBase = class {
      constructor() {
        ...
        let nodeFlag = (_a = process3.env[exports2.env.nodeEnables]) !== null && _a !== void 0 ? _a : "*";
        if (nodeFlag === "all") {
          nodeFlag = "*";
        }
    ...
    function log(namespace, parent) {
      const enablesFlag = process3.env[exports2.env.nodeEnables];
      if (!enablesFlag) {
  ...
  "node_modules/gcp-metadata/build/src/index.js"(exports2) {
    ...
    async function fastFailMetadataRequest(options) {
      ...
      const secondaryOptions = {
        ...options,
        url: (_a = options.url) === null || _a === void 0 ? void 0 : _a.toString().replace(getBaseUrl2(), getBaseUrl2(exports2.SECONDARY_HOST_ADDRESS))
      };
      let responded = false;
...
function resolveStateDir(cwd) {
  ...
  const hash = (0, import_node_crypto5.createHash)("sha256").update(workspaceRoot).digest("hex").slice(0, 16);
...
function generateJobId() {
  const ts = Date.now();
  const rand = (0, import_node_crypto5.randomUUID)().slice(0, 8);
  return `job-${ts}-${rand}`;
}
...
var import_node_crypto5, import_node_fs10, import_node_path12, import_node_os2, MAX_JOBS, PLUGIN_DATA_ENV, SESSION_ID_ENV, FALLBACK_STATE_ROOT;
...
var OAUTH_CREDS_PATH = (0, import_node_path.join)((0, import_node_os.homedir)(), ".gemini", "oauth_creds.json");
var OAUTH_SCOPES = [
  "https://www.googleapis.com/auth/cloud-platform",
...
async function createAuth() {
  if ((0, import_node_fs.existsSync)(OAUTH_CREDS_PATH)) {
    let credentials;
    try {
      credentials = JSON.parse((0, import_node_fs.readFileSync)(OAUTH_CREDS_PATH, "utf-8"));
    } catch (err) {
      throw new Error(`Failed to parse ${OAUTH_CREDS_PATH}: ${String(err)}`);
    }
    const client = new import_google_auth_library.OAuth2Client(OAUTH_CLIENT_ID, OAUTH_CLIENT_SECRET);
  ...
  try {
    const auth = new import_google_auth_library.GoogleAuth({ scopes: OAUTH_SCOPES });
    const authClient = await auth.getClient();
    let oauthClient;
    if (authClient instanceof import_google_auth_library.OAuth2Client) {
  ...
  throw new Error(
    "No Gemini authentication found. Please run `gemini auth login`, set GOOGLE_APPLICATION_CREDENTIALS, or set GEMINI_API_KEY."
  );
}
...
async function* parseSSEStream(stream) {
  const rl = (0, import_node_readline.createInterface)({
    input: import_node_stream.Readable.from(stream),
    crlfDelay: Infinity
  });
  let bufferedLines = [];
...
var CodeAssistClient = class {
  ...
  async init() {
    ...
    try {
      const loadRes = await this.post("loadCodeAssist", {
      ...
      const onboardRes = await this.post("onboardUser", {
        tierId,
        cloudaicompanionProject: void 0,
        metadata
      });
      if (onboardRes.name && !onboardRes.done) {
        for (let i = 0; i < 12; i++) {
          ...
          const opRes = await this.get(onboardRes.name);
          if (opRes.done) {
      ...
      this.initialized = true;
      if (process.env["DEBUG"]) {
        console.error(`[CodeAssist] Init failed: ${err.message}`);
      }
    }
  }
  ...
  async *generateContentStream(params) {
    ...
    for await (const chunk of parseSSEStream(res.data)) {
...
async function executeListDirectory(args, cwd) {
  const dirPath = (0, import_node_path2.resolve)(cwd, String(args["dir_path"] ?? "."));
  const ignorePatterns = args["ignore"] ?? [];
  let entries;
  try {
    entries = (0, import_node_fs2.readdirSync)(dirPath);
  } catch (err) {
    return `Error: Cannot read directory '${dirPath}': ${err.message}`;
  }
  entries = entries.filter((name) => !PRUNE_DIRS.has(name));
  if (ignorePatterns.length > 0) {
    const regexes = ignorePatterns.map(
  ...
  for (const name of entries.sort()) {
    try {
      const fullPath = (0, import_node_path2.join)(dirPath, name);
...
var readFileDeclaration = {
  name: "read_file",
  description: "Reads and returns the content of a specified file. If the file is large, the content will be truncated. Use 'start_line' and 'end_line' parameters to read specific line ranges.",
  parameters: {
    type: "object",
    properties: {
      file_path: {
...
async function executeReadFile(args, cwd) {
  const filePath = (0, import_node_path3.resolve)(cwd, String(args["file_path"] ?? ""));
  ...
  let content;
  try {
    content = (0, import_node_fs3.readFileSync)(filePath, "utf-8");
  } catch (err) {
  ...
  if (endLine !== void 0 && endLine >= rangeStart) {
...
var MAX_RESULTS = 200;
function walkAndMatch(dir, pattern, results, depth = 0) {
  ...
  for (const name of entries) {
    if (PRUNE_DIRS2.has(name) || name.startsWith(".") && depth === 0 && name !== ".") continue;
    const fullPath = (0, import_node_path4.join)(dir, name);
    let stat;
    try {
...
async function executeGlob(args, cwd) {
  const pattern = String(args["pattern"] ?? "");
  const dirPath = (0, import_node_path4.resolve)(cwd, String(args["dir_path"] ?? cwd));
  if (!pattern) {
...
var grepSearchDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      ...
      names_only: {
        type: "boolean",
        description: "Optional: If true, only the file paths of the matches will be returned, without the line content."
      }
    },
    required: ["pattern"]
  }
...
var TIMEOUT_MS = 3e4;
async function executeGrepSearch(args, cwd) {
  const pattern = String(args["pattern"] ?? "");
  const dirPath = (0, import_node_path5.resolve)(cwd, String(args["dir_path"] ?? cwd));
  const includePattern = args["include_pattern"];
  const namesOnly = args["names_only"];
  if (!pattern) {
    return "Error: pattern is required.";
  }
  const hasRg = (() => {
    try {
      (0, import_node_child_process.execSync)("which rg", { stdio: "pipe" });
      return true;
    } catch {
      return false;
    }
  ...
  let output;
  try {
    output = (0, import_node_child_process.execSync)(cmd, {
      cwd: dirPath,
      timeout: TIMEOUT_MS,
      maxBuffer: 1024 * 1024,
      encoding: "utf-8",
...
var MAX_CONTENT_LENGTH2 = 25e4;
var USER_AGENT = "Mozilla/5.0 (compatible; GeminiPlugin/1.0)";
var webFetchDeclaration = {
  name: "web_fetch",
  description: "Fetches content from URL(s) embedded in a prompt. Include up to 20 URLs and instructions (e.g., summarize, extract specific data) directly in the prompt parameter.",
  parameters: {
    type: "object",
    properties: {
      prompt: {
        type: "string",
        description: 'A comprehensive prompt that includes the URL(s) (up to 20) to fetch and specific instructions on how to process their content. All URLs must start with "http://" or "https://".'
      }
    },
    required: ["prompt"]
  }
...
function stripHtml(html) {
  return html.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "").replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "").replace(/<\/(p|div|h[1-6]|li|tr|br\s*\/?)>/gi, "\n").replace(/<br\s*\/?>/gi, "\n").replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/g, " ").replace(/[ \t]+/g, " ").replace(/\n{3,}/g, "\n\n").trim();
}
...
async function fetchUrl(url) {
  try {
    ...
    return { url, content: truncate(content, MAX_CONTENT_LENGTH2) };
  } catch (err) {
    return { url, error: err.message };
  }
}
async function executeWebFetch(args, _cwd) {
...
var outputSchema = {
  ...
  properties: {
    SummaryOfFindings: {
      type: "string",
      description: "A summary of the investigation's conclusions and insights for the main agent."
    },
    ExplorationTrace: {
      type: "array",
      description: "A step-by-step list of actions and tools used during the investigation.",
      items: { type: "string" }
    },
    RelevantLocations: {
      type: "array",
...
var listCommand = process.platform === "win32" ? "`dir /s` (CMD) or `Get-ChildItem -Recurse` (PowerShell)" : "`ls -R`";
var SYSTEM_PROMPT = `You are **Codebase Investigator**, a hyper-specialized AI agent and an expert in reverse-engineering complex software projects. You are a sub-agent within a larger development system.
...
function createInvestigatorConfig(objective, cwd, useCodeAssist) {
  return {
    model: useCodeAssist ? CODE_ASSIST_MODEL : STANDARD_FALLBACK_MODEL2,
    fallbackModel: STANDARD_FALLBACK_MODEL2,
...
// src/agents/complete-task.ts
var COMPLETE_TASK_TOOL_NAME = "complete_task";
function buildCompleteTaskDeclaration(outputConfig) {
  if (outputConfig) {
    return {
      name: COMPLETE_TASK_TOOL_NAME,
      description: "Call this tool to submit your final answer and complete the task. You MUST call this tool when your investigation is complete.",
      parameters: {
        type: "object",
        properties: {
  ...
  return {
    name: COMPLETE_TASK_TOOL_NAME,
    description: "Call this tool to submit your final findings and complete the task. You MUST call this tool when your investigation is complete.",
    parameters: {
      type: "object",
      properties: {
        result: {
...
var TIMEOUT_WARNING = "You have exceeded the time limit. You have one final chance to call the `complete_task` tool with your findings so far. Summarize what you have discovered and call `complete_task` immediately.";
var MAX_TURNS_WARNING = "You have reached the maximum number of turns. You have one final chance to call the `complete_task` tool with your findings so far. Summarize what you have discovered and call `complete_task` immediately.";
var NO_TOOL_CALL_WARNING = "You must use tools to investigate the codebase. Call one of the available tools, or if you are done, call the `complete_task` tool with your findings.";
function progress2(message) {
  const time = (/* @__PURE__ */ new Date()).toLocaleTimeString("en-US", { hour12: false });
  console.error(`[${time}] ${message}`);
}
      ...
      if (candidate.content?.parts) {
        for (const part of candidate.content.parts) {
          if (part.text !== void 0 && !part.thought) {
            ...
            if (lastPart && lastPart.text !== void 0 && !lastPart.thought && !lastPart.functionCall && !lastPart.functionResponse) {
  ...
  while (turnCount < config.maxTurns) {
    ...
    for (const part of modelParts) {
      if (part.thought && part.text) {
        ...
        progress2(`  [thinking] ${preview}`);
      }
    }
    if (functionCalls.length === 0) {
      progress2("  No tool calls received. Nudging...");
    ...
    for (const { call, id } of functionCalls) {
      if (call.name === COMPLETE_TASK_TOOL_NAME) {
        ...
        if (outputValue === void 0 || outputValue === null) {
          progress2(`  complete_task called without '${outputName}' argument. Asking for retry.`);
          responseParts.push({
            functionResponse: {
              id,
              name: COMPLETE_TASK_TOOL_NAME,
        ...
        const resultStr = typeof outputValue === "string" ? outputValue : JSON.stringify(outputValue, null, 2);
        progress2("Investigation complete.");
        return { result: resultStr, terminateReason: "GOAL" };
      }
      progress2(`  [tool] ${call.name}(${summarizeArgs(call.args)})`);
  ...
  try {
    ...
    for (const { call } of functionCalls) {
      if (call.name === COMPLETE_TASK_TOOL_NAME) {
        ...
        if (outputValue !== void 0 && outputValue !== null) {
          const resultStr = typeof outputValue === "string" ? outputValue : JSON.stringify(outputValue, null, 2);
          progress2("Recovery turn succeeded.");
          return { result: resultStr, terminateReason: "GOAL" };
        }
      }
    }
  } catch (err) {
    progress2(`Recovery turn failed: ${err.message}`);
  }
  return null;
}
...
function renderInvestigationReport(raw) {
  ...
  if (report.RelevantLocations && report.RelevantLocations.length > 0) {
    ...
    for (const loc of report.RelevantLocations) {
      const symbols = loc.KeySymbols?.length ? loc.KeySymbols.map((s) => `\`${s}\``).join(", ") : "";
...
async function runInvestigate(objective, cwd, options = {}) {
  ...
  if (writePath) {
    const outPath = (0, import_node_path6.resolve)(cwd, writePath);
    (0, import_node_fs5.mkdirSync)((0, import_node_path6.dirname)(outPath), { recursive: true });
    (0, import_node_fs5.writeFileSync)(outPath, rendered, "utf-8");
    progress3(`Report saved to: ${outPath}`);
  }
...
function hashContent(body) {
  return (0, import_node_crypto4.createHash)("sha256").update(body, "utf-8").digest("hex").slice(0, 12);
}
